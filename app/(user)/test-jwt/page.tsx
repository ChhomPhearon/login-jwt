...
const TestJWTPage = () => {
    ...
    const handleLogin = async () => {
        const email = "chhomphearon1902@gmail.com";
        const password = "Phearon12345";
    ...
    const handleUpdate = async () => {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}products/${499}/`,{
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
        ...
        console.log("Response When Updated",data);
        if (res.status === 401) {
   ...
   const handleRefreshToken = async () => {
    ...
    return (
        <main className="h-screen grid place-content-center">
            <h1 className="text-4xl font-bold text-center"> TEST HANDEL LOGIN</h1>
            <button className="my-4 px-10 py-3 bg-blue-600 rounded-xl text-gray-100 text-3xl"
                    onClick={handleLogin}>Login
            </button>
            {/*refresh token automatically*/}
            <button className="my-4 px-10 py-3 bg-blue-600 rounded-xl text-gray-100 text-3xl"
                    onClick={handleUpdate}>Update Token
            </button>
